...
export default function Upload() {
  ...
  async function handleBulkUpload() {
    ...
    reader.onload = async (evt) => {
      ...
      const { error, count } = await supabase.from('responses').upsert(payload, { onConflict: 'project_id,uid', count: 'exact' })
      setBulkBusy(false)
      if (error) {
        setBulkMessage({ type: 'error', text: error.message })
      } else {
        setBulkMessage({ type: 'success', text: `${payload.length} respondent rows uploaded to ${bulkProjectId}.` })
        setFile(null)
        setPreview([])
      }
    }
