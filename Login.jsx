...
export default function ProjectsAdmin() {
  ...
  function getGenericTrackingLinks() {
    const pid = genericProjectId.trim()
    if (!pid) return null
    const token = encodeTrackToken(pid, null, null)
    return [
  ...
  function copyAllGenericLinks() {
    ...
    if (!links) return
    const formatted = [
      `PackTalk Tracking Links — ${genericProjectId.trim()}`,
      '',
      ...links.map((l) => `${l.label}: ${l.url}`),
      '',
      "Replace [UID] with your respondent ID variable.",
      'The Project ID must be created in PackTalk before the survey goes live, or hits will not be logged.',
    ].join('\n')
    ...
    <div className="page">
      ...
      <div className="card" style={{ maxWidth: 720, marginBottom: 20 }}>
        ...
        <p className="card-hint">
          Send these to a new client before their project even exists in PackTalk. Enter the agreed Project ID below and the links update automatically —{' '}
          <code>[UID]</code> is the only thing left for the client to fill in, with their respondent ID variable.
          Just make sure the Project ID is created below before the survey actually goes live — hits for a Project ID that doesn't exist yet won't be logged.
        </p>
        <div style={{ marginBottom: 12 }}>
          ...
          <input
            value={genericProjectId}
            onChange={(e) => setGenericProjectId(e.target.value)}
            placeholder="e.g. COIN658"
            style={{ maxWidth: 260 }}
          />
        </div>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', marginBottom: 8 }}>
          <button type="button" className="btn-ghost" onClick={copyAllGenericLinks} disabled={!genericProjectId.trim()}>
            {copiedKey === 'generic_all' ? 'Copied All ✓' : 'Copy All Links'}
          </button>
        </div>
        {genericProjectId.trim() ? (
          ...
          <p className="card-hint">Enter a Project ID above to generate the links.</p>
        )}
      </div>
      </Reveal>
      ...
      <div className="card">
        ...
        {linksProjectId && (
          <div className="card" style={{ marginTop: 16, background: 'rgba(255,255,255,0.02)' }}>
            ...
            <p className="card-hint">
              Replace <code>[UID]</code> in each link with your survey tool's dynamic respondent ID variable before handing it to a client or embedding it as a redirect URL. The project is identified by the link itself, so no Project ID, Country, or Age Band appears in the URL.
            </p>
